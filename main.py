...
class Share:
    ...
    async def share(self, session, token, cookie, post_id, share_count):
        ...
        while count < share_count:
            async with session.post(f'https://graph.facebook.com/me/feed?link=https://mbasic.facebook.com/{post_id}&published=0&access_token={token}', headers=headers) as response:
